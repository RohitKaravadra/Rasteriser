...
float4 Pixel(PS_INPUT input) : SV_Target0
{
    ...
    // Determine if the point is outside the light frustum
    bool outsideFrustum = lightClip.x < 0 || lightClip.x > 1 || lightClip.y < 0 || lightClip.y > 1;

    // Calculate shadow 
    float shadow = outsideFrustum || lightClip.z >= 1 ? 1.0 : (lightClip.z > shadowDepth ? 0.2 : 1.0);
